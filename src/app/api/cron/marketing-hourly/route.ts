// Marketing Hourly Cron — runs email sequences and nurture follow-ups.
// Vercel Cron schedule: "0 * * * *" (every hour)
// Protected by CRON_SECRET bearer token.

import { timingSafeEqual } from "node:crypto";
import { NextResponse } from "next/server";
import { runHourly } from "@/marketing/conductor";
...
function authorized(request: Request): boolean {
  ...
  if (!secret) return true; // dev/mock only
...
async function handleCron(request: Request) {
  ...
  try {
    ...
    logger.info("cron.marketing_hourly.done", { elapsedMs: elapsed });
    return NextResponse.json({ ok: true, elapsedMs: elapsed });
  } catch (error) {
    await captureError("cron.marketing_hourly.failed", error);
